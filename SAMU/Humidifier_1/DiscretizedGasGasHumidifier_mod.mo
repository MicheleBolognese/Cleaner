...
model DiscretizedGasGasHumidifier_mod
  ...
  .FuelCell.Pipes.FlowChannel shell(
    ...
    H_flow = h_water_sec .* m_trans_shell,    //mod by sam enthalpy is the one of the vapor that is being transferred - coming from tubes side
...
//  .Modelon.Media.Units.EnthalpyFlowRate[n] h_water_prim "enthalpy of gaseous water"; /mod by sam
  .Modelon.Media.Units.EnthalpyFlowRate[n] h_water_sec "enthalpy of gaseous water";
...
//     h_water_sec_l[i] = SecondaryMedium.enthalpyOfLiquid(state_sec[i].T);
    ...
    m_trans[i] = (Mv*A_mem/(0.5*t_mem)) * (Dw[i]*(C_tube[i] - C_shell[i])); //mod for consistency with vapor coming from tube
...
end DiscretizedGasGasHumidifier_mod;
