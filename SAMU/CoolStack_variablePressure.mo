...
model CoolStack_variablePressure "Cooled PEMFC stack test model"
  ...
  .Modelica.Electrical.Analog.Basic.Ground ground annotation (Placement(transformation(extent={{30.0,52.0},{42.0,64.0}},   rotation=
           0.0,origin = {0.0,0.0})));
  ...
  .Modelica.Blocks.Math.Add p_water_out(k2 = -1,k1 = +1) annotation(Placement(transformation(extent = {{62.0,-104.0},{82.0,-84.0}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Modelica.Blocks.Sources.CombiTimeTable dp_an_variation(startTime = 0,offset = {0},columns = {2},tableOnFile = false,table = {{0,4000.00},{225,4000.00},{450,4000.00},{675,4000.00},{900,4000.00},{1125,4000.00},{1350,4000.00},{1575,4000.00},{1800,4000.00},{2025,4000.00},{2250,6666.55},{2475,9333.45},{2700,11111.03},{2925,12000.00},{3150,14666.55},{3375,17333.45},{3600,20000.00}}) annotation(Placement(transformation(extent = {{-211.4,146.6},{-188.6,169.4}},origin = {0.0,0.0},rotation = 0.0)));
    .Modelica.Blocks.Sources.CombiTimeTable p_an_variation(table = {{0,141325.0},{225,141325.0},{450,141325.0},{675,141325.0},{900,141325.0},{1125,151325.0},{1350,151325.0},{1575,151325.0},{1800,191325.0},{2025,191325.0},{2250,221325.0},{2475,221325.0},{2700,221325.0},{2925,221325.0},{3150,221325.0},{3375,221325.0},{3600,221325.0}},tableOnFile = false,columns = {2},offset = {0},startTime = 0) annotation(Placement(transformation(extent = {{-211.4,84.6},{-188.6,107.4}},origin = {0.0,0.0},rotation = 0.0)));
    .Modelica.Blocks.Sources.CombiTimeTable flow_rate_an_variation(startTime = 0,offset = {0},columns = {2},tableOnFile = false,table = {{0,29.59},{225,29.59},{450,29.59},{675,29.59},{900,29.59},{1125,29.59},{1350,29.59},{1575,29.59},{1800,29.59},{2025,29.59},{2250,36.987},{2475,44.385},{2700,49.316},{2925,51.782},{3150,59.179},{3375,66.577},{3600,73.974}}) annotation(Placement(transformation(extent = {{-211.4,30.6},{-188.6,53.4}},origin = {0.0,0.0},rotation = 0.0)));
    .Modelica.Blocks.Sources.CombiTimeTable flow_rate_cath_variation(columns = {2},startTime = 0,offset = {0},table = {{0,59.23},{225,59.23},{450,59.23},{675,59.23},{900,59.23},{1125,59.23},{1350,59.23},{1575,59.23},{1800,59.23},{2025,59.23},{2250,74.038},{2475,88.845},{2700,98.717},{2925,103.653},{3150,118.461},{3375,133.268},{3600,148.076}},tableOnFile = false) annotation(Placement(transformation(extent = {{-211.4,-31.4},{-188.6,-8.6}},origin = {0.0,0.0},rotation = 0.0)));
    .Modelica.Blocks.Sources.CombiTimeTable p_cath_variation(startTime = 0,offset = {0},columns = {2},tableOnFile = false,table = {{0,121325.0},{225,121325.0},{450,121325.0},{675,121325.0},{900,121325.0},{1125,131325.0},{1350,131325.0},{1575,131325.0},{1800,171325.0},{2025,171325.0},{2250,201325.0},{2475,201325.0},{2700,201325.0},{2925,201325.0},{3150,201325.0},{3375,201325.0},{3600,201325.0}}) annotation(Placement(transformation(extent = {{-211.4,-93.4},{-188.6,-70.6}},origin = {0.0,0.0},rotation = 0.0)));
    .Modelica.Blocks.Sources.CombiTimeTable dp_cath_variation(table = {{0,10000.00},{225,10000.00},{450,10000.00},{675,10000.00},{900,10000.00},{1125,10000.00},{1350,10000.00},{1575,10000.00},{1800,10000.00},{2025,10000.00},{2250,12083.38},{2475,14166.62},{2700,15555.54},{2925,16250.00},{3150,18333.38},{3375,20416.62},{3600,22500.00}},tableOnFile = false,columns = {2},offset = {0},startTime = 0) annotation(Placement(transformation(extent = {{-211.4,-153.4},{-188.6,-130.6}},origin = {0.0,0.0},rotation = 0.0)));
...
equation       
  ...
  connect(dp_water.y,p_water_out.u2) annotation(Line(points = {{41,-114},{50.5,-114},{50.5,-100},{60,-100}},color = {0,0,127}));
    ...
    connect(dp_an_variation.y[1],p_an_out.u1) annotation(Line(points = {{-187.46,158},{-98.73,158},{-98.73,134},{-54,134}},color = {0,0,127}));
