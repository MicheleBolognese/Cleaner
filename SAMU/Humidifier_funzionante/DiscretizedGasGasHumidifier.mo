...
model DiscretizedGasGasHumidifier
  ...
  .FuelCell.Pipes.FlowChannel shell(
    ...
    H_flow = -h_water_prim .* m_trans_shell, // minus added by sam for coherence - atm m_trans has the same sign of m_trans_shell, so the minus hard coded here is needed for energy balance
...
//     h_water_sec_l[i] = SecondaryMedium.enthalpyOfLiquid(state_sec[i].T);
    ...
    // CODICE CORRETTO con spliceFunction a due livelli

    // Transizione smussata tra ramo polinomiale (a<=1) e lineare (1<a<=3)
    lambda_shell[i] = Modelon.Math.Smoothing.spliceFunction(
      pos    = Modelon.Math.Smoothing.spliceFunction(
                 pos    = 16.8,
                 neg    = 14 + 1.4*(a_shell[i] - 1),
                 x      = a_shell[i] - 3,
                 deltax = 0.05),
      neg    = 0.043 + 17.81*a_shell[i] - 39.85*a_shell[i]^2 + 36.0*a_shell[i]^3,
      x      = a_shell[i] - 1,
      deltax = 0.05);

    lambda_tube[i] = Modelon.Math.Smoothing.spliceFunction(
      pos    = Modelon.Math.Smoothing.spliceFunction(
                 pos    = 16.8,
                 neg    = 14 + 1.4*(a_tube[i] - 1),
                 x      = a_tube[i] - 3,
                 deltax = 0.05),
      neg    = 0.043 + 17.81*a_tube[i] - 39.85*a_tube[i]^2 + 36.0*a_tube[i]^3,
      x      = a_tube[i] - 1,
      deltax = 0.05);

    lambda_mem[i] = Modelon.Math.Smoothing.spliceFunction(
      pos    = Modelon.Math.Smoothing.spliceFunction(
                 pos    = 16.8,
                 neg    = 14 + 1.4*(a_mem[i] - 1),
                 x      = a_mem[i] - 3,
                 deltax = 0.05),
      neg    = 0.043 + 17.81*a_mem[i] - 39.85*a_mem[i]^2 + 36.0*a_mem[i]^3,
      x      = a_mem[i] - 1,
      deltax = 0.05);

    
    D[i] = noEvent(
      if lambda_mem[i] < 2 then 1e-6
      elseif lambda_mem[i] >= 2 and lambda_mem[i] <= 3 then
        1e-6*(1 + 2*(lambda_mem[i] - 2))
      elseif lambda_mem[i] > 3 and lambda_mem[i] < 4.5 then
        1e-6*(3 - 5.0/3.0*(lambda_mem[i] - 3))
      else 1.25e-6);
