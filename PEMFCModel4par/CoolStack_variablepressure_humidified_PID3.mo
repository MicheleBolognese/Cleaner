...
model CoolStack_variablepressure_humidified_PID3 "Cooled PEMFC stack test model"
  ...
  parameter .Modelica.Units.SI.Pressure p_an_start = 1.4e5 "Start value of anodic gas pressure at variable load" annotation(Dialog(group = "Anode side", enable = not constant_current));
  /*parameter .Modelica.Units.SI.Pressure p_an_max = 2.2e5 "Maximum value of anodic gas pressure at variable load" annotation(Dialog(group = "Anode side", enable = not constant_current));*/
  parameter .Modelica.Units.SI.Pressure p_an = 1.6e5 "Anodic gas pressure at constant load" annotation(Dialog(group = "Anode side", enable = constant_current));      
  /*parameter .Modelica.Units.SI.Pressure dp_an_nom = 0.12e5 "Nominal pressure drop at anode side" annotation(Dialog(group = "Anode side"));*/
  ...
  parameter .Modelica.Units.SI.Pressure p_cath_start = 1.2e5 "Start value of cathodic gas pressure at variable load" annotation(Dialog(group = "Cathode side", enable = not constant_current));
  /*parameter .Modelica.Units.SI.Pressure p_cath_max = 2.2e5 "Maximum value of cathodic gas pressure at variable load" annotation(Dialog(group = "Cathode side", enable = not constant_current));*/
  parameter .Modelica.Units.SI.Pressure p_cath = 1.6e5 "Cathodic gas pressure at constant load" annotation(Dialog(group = "Cathode side", enable = constant_current));  
  ...
  parameter Real V_flow_water_min(unit = "LPM") = 4.95 "Minimum value of water inlet volumetric flow rate at variable load" annotation(Dialog(group = " Cooling", enable = not constant_current));
  parameter Real V_flow_water_max(unit = "LPM") = 4.95 "Maximum value of water inlet volumetric flow rate at variable load" annotation(Dialog(group = " Cooling", enable = not constant_current));
  parameter Real V_flow_water(unit = "LPM") = 4.95 "Water inlet volumetric flow rate at constant load" annotation(Dialog(group = " Cooling", enable = constant_current));      
  ...
  parameter .Modelica.Units.SI.Pressure p_water_start = 1.6e5 "Start value of water pressure at variable load" annotation(Dialog(group = " Cooling", enable = not constant_current));
  /*parameter .Modelica.Units.SI.Pressure p_water_max = 2.4e5 "Maximum values of water pressure at variable load" annotation(Dialog(group = " Cooling", enable = not constant_current));*/
  parameter .Modelica.Units.SI.Pressure p_water = 2.4e5 "Water pressure at constant load" annotation(Dialog(group = " Cooling", enable = constant_current));
  ...
  /*parameter .Modelica.Units.SI.Current I_min = 1 "Minimum value of load current at variable load" annotation(Dialog(group = "Load", enable = not constant_current));
  parameter .Modelica.Units.SI.Current I_max = 450 "Maximum value of load current at variable load" annotation(Dialog(group = "Load", enable = not constant_current));*/
  parameter .Modelica.Units.SI.Current I = 180 "Load current at constant load" annotation(Dialog(group = "Load", enable = constant_current));
  parameter Boolean constant_current = false "If true, the load current is constant" annotation(Dialog(group = "Load"));
  /*parameter Boolean variable_current_timeTable = true "If true, the load current varies according to a time-table" annotation(Dialog(group = "Load"));
  parameter Boolean variable_current_linear = false "If true, the load current varies linearly" annotation(Dialog(group = "Load"));*/
  ...
  parameter Real m_flow_cath_nom = 6.072e-4 "Nominal value of cathodic wet gas inlet mass flow rate for linear friction loss model" annotation(Dialog(tab = "Nominal conditions", group = "Cathode side"));
  ...
  parameter Real rho_cath_nom = 0.80 "Nominal value of cathodic wet gas inlet density for linear friction loss model"  annotation(Dialog(tab = "Nominal conditions", group = "Cathode side"));
  ...
  .Modelica.Blocks.Sources.CombiTimeTable current_variation(tableOnFile = false,table = {{0,1},{225,9},{450,22.5},{675,45},{900,60},{1125,67.5},{1350,90},{1575,112.5},{1800,135},{2025,180},{2250,225},{2475,270},{2700,300},{2925,315},{3150,360},{3375,405},{3600,450}},verboseExtrapolation = false,columns = {2},timeScale = 1) annotation(Placement(transformation(extent = {{-50.84161487909551,91.15838512090448},{-37.15838512090449,104.84161487909552}},origin = {0.0,0.0},rotation = 0.0)));
  .Modelica.Electrical.Analog.Sources.SignalCurrent variable_load if not constant_current annotation(Placement(transformation(extent = {{-5.033959333391412,60.96604066660859},{9.033959333391412,75.03395933339141}},origin = {0.0,0.0},rotation = 0.0)));
  .Modelica.Electrical.Analog.Sources.ConstantCurrent constant_load(I = I) if constant_current annotation(Placement(transformation(origin = {2.0,54.0},extent = {{7.0,-7.0},{-7.0,7.0}},rotation = -180.0)));
  .Modelica.Blocks.Sources.CombiTimeTable flow_rate_an_variation(table = {{0,29.59},{225,29.59},{450,29.59},{675,29.59},{900,29.59},{1125,29.59},{1350,29.59},{1575,29.59},{1800,29.59},{2025,29.59},{2250,36.987},{2475,44.385},{2700,49.316},{2925,51.782},{3150,59.179},{3375,66.577},{3600,73.974}},tableOnFile = false,columns = {2},offset = {0},startTime = 0,timeScale = 1) annotation(Placement(transformation(extent = {{-203.48781459666964,24.512185403330363},{-180.51218540333036,47.48781459666964}},origin = {0.0,0.0},rotation = 0.0)));
  .Modelica.Blocks.Sources.CombiTimeTable flow_rate_cath_variation(tableOnFile = false,table = {{0,59.23},{225,59.23},{450,59.23},{675,59.23},{900,59.23},{1125,59.23},{1350,59.23},{1575,59.23},{1800,59.23},{2025,59.23},{2250,74.038},{2475,88.845},{2700,98.717},{2925,103.653},{3150,118.461},{3375,133.268},{3600,148.076}},offset = {0},startTime = 0,columns = {2},timeScale = 1) annotation(Placement(transformation(extent = {{-211.48781459666964,-51.48781459666964},{-188.51218540333036,-28.512185403330363}},origin = {0.0,0.0},rotation = 0.0)));
    
  .Modelica.Blocks.Sources.CombiTimeTable V_cell_exp(table = {{0,0.969},{225,0.879},{450,0.844},{675,0.809},{900,0.793},{1125,0.791},{1350,0.777},{1575,0.767},{1800,0.762},{2025,0.742},{2250,0.734},{2475,0.718},{2700,0.708},{2925,0.703},{3150,0.687},{3375,0.672},{3600,0.656}},columns = {2},timeScale = 1) annotation(Placement(transformation(extent = {{-382.5,79.0},{-362.5,99.0}},origin = {0.0,0.0},rotation = 0.0)));
  .Modelica.Blocks.Sources.CombiTimeTable p_an_variation(startTime = 0,offset = {0},columns = {2},tableOnFile = false,table = {{0,141325.0},{225,141325.0},{450,141325.0},{675,141325.0},{900,141325.0},{1125,151325.0},{1350,151325.0},{1575,151325.0},{1800,191325.0},{2025,191325.0},{2250,221325.0},{2475,221325.0},{2700,221325.0},{2925,221325.0},{3150,221325.0},{3375,221325.0},{3600,221325.0}},timeScale = 1) annotation(Placement(transformation(extent = {{-203.48781459666964,72.51218540333036},{-180.51218540333036,95.48781459666964}},origin = {0.0,0.0},rotation = 0.0)));
  .Modelica.Blocks.Sources.CombiTimeTable dp_an_variation(table = {{0,4000.00},{225,4000.00},{450,4000.00},{675,4000.00},{900,4000.00},{1125,4000.00},{1350,4000.00},{1575,4000.00},{1800,4000.00},{2025,4000.00},{2250,6666.55},{2475,9333.45},{2700,11111.03},{2925,12000.00},{3150,14666.55},{3375,17333.45},{3600,20000.00}},tableOnFile = false,columns = {2},offset = {0},startTime = 0,timeScale = 1) annotation(Placement(transformation(extent = {{-147.48781459666964,136.51218540333036},{-124.51218540333036,159.48781459666964}},origin = {0.0,0.0},rotation = 0.0)));
  .Modelica.Blocks.Sources.CombiTimeTable p_cath_variation(table = {{0,121325.0},{225,121325.0},{450,121325.0},{675,121325.0},{900,121325.0},{1125,131325.0},{1350,131325.0},{1575,131325.0},{1800,171325.0},{2025,171325.0},{2250,201325.0},{2475,201325.0},{2700,201325.0},{2925,201325.0},{3150,201325.0},{3375,201325.0},{3600,201325.0}},tableOnFile = false,columns = {2},offset = {0},startTime = 0,timeScale = 1) annotation(Placement(transformation(extent = {{-195.48781459666964,-123.48781459666964},{-172.51218540333036,-100.51218540333036}},origin = {0.0,0.0},rotation = 0.0)));
  .Modelica.Blocks.Sources.CombiTimeTable dp_cath_variation(startTime = 0,offset = {0},columns = {2},tableOnFile = false,table = {{0,10000.00},{225,10000.00},{450,10000.00},{675,10000.00},{900,10000.00},{1125,10000.00},{1350,10000.00},{1575,10000.00},{1800,10000.00},{2025,10000.00},{2250,12083.38},{2475,14166.62},{2700,15555.54},{2925,16250.00},{3150,18333.38},{3375,20416.62},{3600,22500.00}},timeScale = 1) annotation(Placement(transformation(extent = {{-165.48781459666964,-183.48781459666964},{-142.51218540333036,-160.51218540333036}},origin = {0.0,0.0},rotation = 0.0)));
  .Modelica.Blocks.Sources.CombiTimeTable p_water_variation(startTime = 0,offset = {0},columns = {2},tableOnFile = false,table = {{0,160000},{225,160000},{450,160000},{675,160000},{900,160000},{1125,170000},{1350,170000},{1575,170000},{1800,210000},{2025,210000},{2250,240000},{2475,240000},{2700,240000},{2925,240000},{3150,240000},{3375,240000},{3600,240000}},timeScale = 1) annotation(Placement(transformation(extent = {{18.512185403330363,-97.48781459666964},{41.48781459666964,-74.51218540333036}},origin = {0.0,0.0},rotation = 0.0)));
  ...
  .Modelica.Blocks.Math.Add p_water_out(k2 = -1,k1 = +1) annotation(Placement(transformation(extent = {{62.0,-104.0},{82.0,-84.0}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Modelica.Blocks.Math.Gain gain(k = 1 / coolStack.n_cell) annotation(Placement(transformation(extent = {{-370.844731741886,40.655268258114035},{-358.155268258114,53.344731741885965}},origin = {0.0,0.0},rotation = 0.0)));
    .Modelica.Blocks.Continuous.LimPID PID_c1(controllerType = .Modelica.Blocks.Types.SimpleController.P,yMax = 9e-3,yMin = 8.5e-6,k = 1e-3,initType = .Modelica.Blocks.Types.Init.InitialState) annotation(Placement(transformation(extent = {{-290.5,79.0},{-270.5,99.0}},origin = {0.0,0.0},rotation = 0.0)));
    .Modelica.Blocks.Continuous.LimPID PID_j0(controllerType = .Modelica.Blocks.Types.SimpleController.P,yMin = 1e-5,yMax = 100,k = 1e-3,initType = .Modelica.Blocks.Types.Init.InitialState) annotation(Placement(transformation(extent = {{-288.5,13.0},{-268.5,33.0}},origin = {0.0,0.0},rotation = 0.0)));
...
equation       
  ...
  connect(dp_water.y,p_water_out.u2) annotation(Line(points = {{41,-114},{50.5,-114},{50.5,-100},{60,-100}},color = {0,0,127}));
    ...
    connect(voltageSensor.v,gain.u) annotation(Line(points = {{4,120.5630969421064},{4,198},{-410,198},{-410,47},{-372.1136780902632,47}},color = {0,0,127}));
    connect(V_cell_exp.y[1],PID_c1.u_s) annotation(Line(points = {{-361.5,89},{-292.5,89}},color = {0,0,127}));
    connect(gain.y,PID_c1.u_m) annotation(Line(points = {{-357.52079508392535,47},{-280.5,47},{-280.5,77}},color = {0,0,127}));
    connect(V_cell_exp.y[1],PID_j0.u_s) annotation(Line(points = {{-361.5,89},{-326,89},{-326,23},{-290.5,23}},color = {0,0,127}));
    connect(gain.y,PID_j0.u_m) annotation(Line(points = {{-357.52079508392535,47},{-354.155268258114,47},{-354.155268258114,5},{-278.5,5},{-278.5,11}},color = {0,0,127}));
...
end CoolStack_variablepressure_humidified_PID3;
