...
model PEMFCEmpirical "PEMFC empirical activation loss"
  ...
  input .Modelica.Units.SI.CurrentDensity j_loss (start = 100) "Current density loss" annotation(Dialog(group = "Input parameters"));
  input .Modelica.Units.SI.CurrentDensity j_0 (start = 1) "Exchange current density" annotation(Dialog(group = "Input parameters"));
...
equation
  ...
  for i in 1:N loop
    ...
    E_loss_cell[i] = .Modelica.Constants.R*T_cell[i]/(n_e*alpha*.FuelCell.Internal.Units.F)*.Modelica.Math.log(max((j_ionic[i] + j_loss)/j_0,.Modelica.Constants.eps));
