...
model Empirical "PEMFC | Empirical models"
  extends .FuelCell.Membranes.Templates.PartialMembrane(
    final S_ionic_an=Medium_an.stoichiometry({"H2"}, {1}),
    final S_ionic_cath=Medium_cath.stoichiometry({"H2O","O2"}, {-1,0.5}),
    final an_names={"H2","H2O"},
    final cath_names={"O2","H2O"},
    redeclare model ConcentrationLoss =
        .FuelCell.Membranes.Losses.ConcentrationLoss.PEMFCExponentialFit (                                m=m, n=n),
    redeclare model OhmicLoss =
        .FuelCell.Membranes.Losses.OhmicLoss.PEMFCElectroChem (
        z=z,
        T_cell=T_cell,
        pH2O=pH2O,
        pH2O_an=pH2O_an),
    redeclare model ActivationLoss =
        .FuelCell.Membranes.Losses.ActivationLoss.PEMFCEmpirical (
        alpha=alpha,
        j_n=j_n,
        j_0=j_0,
        T_cell=T_cell),z = 75e-6,redeclare replaceable model ElectroOsmoticDrag = .FuelCell.Membranes.MassTransport.ElectroOsmoticDrag.PolyWaterContent(coeffs = {0,0.05,0.0029}));

  parameter Real alpha=0.5 "Charge transfer coefficient" annotation (Dialog(group="Characteristics and polarization"));
  parameter .Modelica.Units.SI.CurrentDensity j_0=1 "Exchange current density, cathode"
    annotation (Dialog(group="Characteristics and polarization"));
  parameter .Modelica.Units.SI.CurrentDensity j_n=100 "Current density loss"
    annotation (Dialog(group="Characteristics and polarization"));
  parameter .Modelica.Units.SI.Voltage m=0 " Experimentally derived value of m coefficient"
    annotation (Dialog(group="Characteristics and polarization"));
  parameter Real n=3.2e-4 " Experimentally derived value of n coefficient"
    annotation (Dialog(group="Characteristics and polarization"));
...
equation
  ...
  for i in 1:N loop
    E0[i] = E0_ref - 0.85e-3*(T_cell[i] - Tref_25C) + 4.3085e-5*T_cell[i]*(.Modelica.Math.log(pH2[i]/.FuelCell.Internal.Units.pNTP) +
      0.5*.Modelica.Math.log(pH2[i]/.FuelCell.Internal.Units.pNTP));
  end for;
