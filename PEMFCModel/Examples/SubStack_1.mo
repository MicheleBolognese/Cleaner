...
model SubStack_1 "Proton exchange membrane fuel cell substack with prox reactor at anode inlet"
  extends .FuelCell.Stacks.Templates.SubStack(
    fuel_in=max(.Modelica.Constants.eps, anode_channel.portA.m_flow*X_feed_an[ref[1]]/Medium_an.MMX[ref[1]]),redeclare replaceable model Membrane = .PEMFCModel.Examples.Empirical,
    addProxToAnode=false,
    L_anode=0.0813,
    L_cathode=0.0813,redeclare replaceable model Friction_anode = .Modelon.ThermoFluid.FlowChannels.PipeResistances.SinglePhase.DetailedWallFriction,redeclare replaceable model Friction_cathode = .Modelon.ThermoFluid.FlowChannels.PipeResistances.SinglePhase.DetailedWallFriction,
    d0_prox=0.5,
    dp0_prox(displayUnit="kPa") = 100,
    m_flow_nom_prox=1e-3,
    D_anode=0.023,
    D_cathode=0.0445,
    T_start_in_anode=333.15,
    T_start_out_anode=333.15,
    T_start_in_cathode=333.15,
    T_start_out_cathode=333.15,includeCellConduction = false,from_dp_anode = false,from_dp_cathode = false,positiveFlow_anode = true,positiveFlow_cathode = true);
