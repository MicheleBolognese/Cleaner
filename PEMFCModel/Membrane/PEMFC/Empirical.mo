...
model Empirical "PEMFC | Empirical models"
  ...
  extends .PEMFCModel.Membrane.Templates.PartialMembrane(
    ...
    redeclare model ConcentrationLoss = .PEMFCModel.Membrane.Losses.ConcentrationLoss.PEMFCExponentialFit(m = m, n = n), 
    ...
    redeclare model ActivationLoss = .PEMFCModel.Membrane.Losses.ActivationLoss.PEMFCEmpirical(
      alpha = alpha, 
      j_loss = j_loss,
      j_0 = j_0,
      final T_cell = T_cell),
  ...
  parameter Real c1(unit "V/K") = 0.85e-3 "Voltage derivative by temperature" annotation(Dialog(enable = enable_setting,group = "Characteristics and polarization"));
  ...
  parameter .Modelica.Units.SI.CurrentDensity j_0 = 1 "Exchange current density for activation loss" annotation (Dialog(enable = enable_setting,group = "Characteristics and polarization"));
  parameter .Modelica.Units.SI.CurrentDensity j_loss = 100 "Activation current density loss" annotation (Dialog(enable = enable_setting,group = "Characteristics and polarization"));
  parameter .Modelica.Units.SI.Voltage m = 3e-4 "Pre-exponential factor for concentration loss" annotation (Dialog(enable = enable_setting,group = "Characteristics and polarization"));
  parameter Real n = 3.2e-4 "Exponential factor for concentration loss" annotation (Dialog(enable = enable_setting,group = "Characteristics and polarization"));
