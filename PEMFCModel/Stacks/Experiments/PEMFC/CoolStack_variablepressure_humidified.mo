...
model CoolStack_variablepressure_humidified "Cooled PEMFC stack test model"
  ...
  .PEMFCModel.Humidification humidification_cath(
    ...
    T_in = T_cath_in ,
    y_dry_in = y_dry_cath,
    T_dew = T_dew_cath) annotation(Placement(transformation(extent = {{-130.3122469373013,-11.687753062698697},{-117.6877530626987,-24.312246937301303}},origin = {0.0,0.0},rotation = 0.0)));
  ...
  .Modelica.Blocks.Sources.CombiTimeTable flow_rate_cath_variation(tableOnFile = false,table = {{0,59.23},{225,59.23},{450,59.23},{675,59.23},{900,59.23},{1125,59.23},{1350,59.23},{1575,59.23},{1800,59.23},{2025,59.23},{2250,74.038},{2475,88.845},{2700,98.717},{2925,103.653},{3150,118.461},{3375,133.268},{3600,148.076}},offset = {0},startTime = 0,columns = {2}) annotation(Placement(transformation(extent = {{-201,-25.1},{-178,-1.9}},origin = {0.0,0.0},rotation = 0.0)));
  ...
  .Modelica.Blocks.Sources.RealExpression dp_water(y = 0.43e5) annotation(Placement(transformation(extent = {{22.0,-120.0},{42.0,-100.0}},origin = {0.0,0.0},rotation = 0.0)));
  ...
  .Modelica.Blocks.Math.Add p_water_out(k2 = -1,k1 = +1) annotation(Placement(transformation(extent = {{62.0,-104.0},{82.0,-84.0}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Modelica.Blocks.Interaction.Show.RealValue i_stack_Acm2(significantDigits = 3,use_numberPort = false,number = coolStack.subStack.cell.j * 1e4) annotation(Placement(transformation(extent = {{184.07221093731997,-61.92778906268006},{211.92778906268003,-34.07221093731994}},origin = {0.0,0.0},rotation = 0.0)));
    .Modelica.Blocks.Interaction.Show.RealValue p_stack_Wcm2(number = coolStack.summary.P_stack / coolStack.subStack.A_cell / 10000,use_numberPort = false,significantDigits = 3) annotation(Placement(transformation(extent = {{186.07221093731997,-87.92778906268006},{213.92778906268003,-60.07221093731994}},origin = {0.0,0.0},rotation = 0.0)));
...
equation       
  ...
  connect(multiDisplaySensor4.portB, coolStack.feed_cooling) annotation (Line(points={{-20,-40},{-12,-40},{-12,3.5}}, color={0,0,255}));
  ...
  connect(multiDisplaySensor1.portA, coolStack.drain_cooling) annotation (Line(points={{18,-40},{4,-40},{4,3.5}}, color={0,0,255}));
  ...
  connect(gasSensor3.portB, coolStack.feed_an) annotation (Line(points={{-62,30},{-22,30}}, color={255,128,0}));
  ...
  connect(gasSensor2.u, display_an_out.data) annotation (Line(points={{34.05,30.05},{34.05,32},{62,32},{62,37.2941}},
                                                   color={0,0,0}));
  connect(gasSensor2.portA, coolStack.drain_an) annotation (Line(points={{28,30},{14,30}}, color={255,128,0}));
  ...
  connect(gasSensor4.portB, coolStack.feed_cath) annotation (Line(points={{-62,-20},{-22,-20},{-22,10}}, color={255,128,0}));
  ...
  connect(gasSensor1.portA, coolStack.drain_cath) annotation (Line(points={{30,-20},{14,-20},{14,10}}, color={255,128,0}));
  ...
  connect(humidification_an.m_flow_wet_in,flowAnode.m_flow_in) annotation(Line(points = {{-116.40952305789519,32.588021244293536},{-110,32.588021244293536},{-110,46},{-97.93906632502666,46},{-97.93906632502666,39.768425703538625}},color = {0,0,127}));
  connect(humidification_an.x_wet_in,flowAnode.X_in) annotation(Line(points = {{-116.18859441508964,27.91695851069057},{-110.18859441508964,27.91695851069057},{-110.18859441508964,46},{-85.93906632502666,46},{-85.93906632502666,39.768425703538625}},color = {0,0,127}));
  connect(humidification_cath.m_flow_wet_in,flowCathode.m_flow_in) annotation(Line(points = {{-116.40952305789519,-20.588021244293536},{-110,-20.588021244293536},{-110,-4},{-98,-4},{-98,-10}},color = {0,0,127}));
  connect(humidification_cath.x_wet_in,flowCathode.X_in) annotation(Line(points = {{-116.18859441508964,-15.91695851069057},{-110.18859441508964,-15.91695851069057},{-110.18859441508964,-4},{-86,-4},{-86,-10}},color = {0,0,127}));
  connect(ramp_m_flow_water.y,sourceW.m_flow_in) annotation(Line(points = {{-177.14817327951067,-58},{-74,-58},{-74,-30},{-54,-30}},color = {0,0,127}));
  connect(humidification_an.x_wet_in,sinkAnode.X_in) annotation(Line(points = {{-116.18859441508964,27.91695851069057},{-110,27.91695851069057},{-110,84},{83.22993225215629,84},{83.22993225215629,39.6149661260782}},color = {0,0,127}));
  connect(p_an_variation.y[1],humidification_an.p_in) annotation(Line(points = {{-177.35,66},{-156,66},{-156,33.78734816238078},{-131.57469632476156,33.78734816238078}},color = {0,0,127}));
  connect(humidification_cath.x_wet_in,sinkCathode.X_in) annotation(Line(points = {{-116.18859441508964,-15.91695851069057},{-110.18859441508964,-15.91695851069057},{-110.18859441508964,-4},{83.61496612607814,-4},{83.61496612607814,-10.38503387392182}},color = {0,0,127}));
  connect(coolStack.pin_n,ground.p) annotation(Line(points = {{4,36.5},{16,36.5},{16,60},{24,60}},color = {0,0,255}));
  connect(current_variation.y[1],variable_load.i) annotation(Line(points = {{-177.35,102},{2,102},{2,76.44075120006968}},color = {0,0,127}));  
  connect(coolStack.pin_p,variable_load.p) annotation(Line(points = {{-12,36.5},{-12,68},{-5.033959333391412,68}},color = {0,0,255}));
  connect(variable_load.n,coolStack.pin_n) annotation(Line(points = {{9.033959333391412,68},{16,68},{16,36.5},{4,36.5}},color = {0,0,255}));
  connect(flow_rate_an_variation.y[1],humidification_an.V_flow_dry_in) annotation(Line(points = {{-177.3634039436634,26},{-131.57469632476156,26},{-131.57469632476156,26.21265183761922}},color = {0,0,127}));
  ...
  connect(p_cath_out.y,sinkCathode.p_in) annotation(Line(points = {{-33,-130},{112,-130},{112,-4},{95.61496612607814,-4},{95.61496612607814,-10.38503387392182}},color = {0,0,127}));
  connect(p_cath_variation.y[1],humidification_cath.p_in) annotation(Line(points = {{-175.3634039436634,-110},{-151.46905013421247,-110},{-151.46905013421247,-21.78734816238078},{-131.57469632476156,-21.78734816238078}},color = {0,0,127}));
  ...
  connect(p_water_variation.y[1],p_water_out.u1) annotation(Line(points = {{42.6365960563366,-86},{50,-86},{50,-88},{60,-88}},color = {0,0,127}));
  connect(dp_water.y,p_water_out.u2) annotation(Line(points = {{43,-110},{50.5,-110},{50.5,-100},{60,-100}},color = {0,0,127}));
    ...
    connect(flow_rate_cath_variation.y[1],humidification_cath.V_flow_dry_in) annotation(Line(points = {{-176.85,-13.5},{-131.57469632476156,-13.5},{-131.57469632476156,-14.212651837619218}},color = {0,0,127}));
...
end CoolStack_variablepressure_humidified;