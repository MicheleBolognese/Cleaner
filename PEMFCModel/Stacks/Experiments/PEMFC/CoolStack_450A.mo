...
model CoolStack_450A "Cooled PEMFC stack test model"
  ...
  parameter Real V_flow_an_min(unit = "NLPM") = 29.59 "Minimum value of anodic dry gas inlet volumetric flow rate" annotation(Dialog(group = "Anode side"));
  parameter Real V_flow_an_max(unit = "NLPM") = 73.974 "Maximum value of anodic dry gas inlet volumetric flow rate" annotation(Dialog(group = "Anode side"));
  parameter .Modelica.Units.SI.Pressure p_an_in_max = 2.2e5 "Maximum start value of inlet pressure at anode side" annotation(Dialog(group = "Anode side"));
  parameter .Modelica.Units.SI.Pressure p_an_in_max = 2.2e5 "Minimum start inlet pressure at anode side" annotation(Dialog(group = "Anode side"));
  parameter .Modelica.Units.SI.Pressure p_an_out_max = 2.1e5 "Outlet pressure at anode side" annotation(Dialog(group = "Anode side"));
  ...
  parameter Real V_flow_cath_min(unit = "NLPM") = 1.342e-3 "Minimum value of cathodic dry gas inlet volumetric flow rate" annotation(Dialog(group = "Cathode side"));
  parameter Real V_flow_cath_max(unit = "NLPM") = V_flow_cath_min "Maximum value of cathodic dry gas inlet volumetric flow rate" annotation(Dialog(group = "Cathode side"));
  parameter .Modelica.Units.SI.Pressure p_cath_in = 1.7e5 "Start inlet pressure at cathode side" annotation(Dialog(group = "Cathode side"));
  parameter .Modelica.Units.SI.Pressure p_cath_out = 1.6e5 "Outlet pressure at cathode side" annotation(Dialog(group = "Cathode side"));
  ...
  parameter Real V_flow_water_min(unit = "NLPM")  = 8.25e-2 "Minimum value of water inlet volumetric flow rate" annotation(Dialog(group = " Cooling"));
  parameter Real V_flow_water_max(unit = "NLPM") = V_flow_water_min "Maximum value of water inlet volumetric flow rate" annotation(Dialog(group = " Cooling"));
  parameter .Modelica.Units.SI.MassFlowRate m_flow_water_min = V_flow_water_min/1000/60*MediumWater.density_pTX(p_water_in, T_water_in, {1.0})  annotation(Dialog(group = " Cooling", enable = false));
  parameter .Modelica.Units.SI.Temperature T_water_in = 65+273.15 "Water inlet temperature" annotation(Dialog(group = " Cooling"));
  parameter .Modelica.Units.SI.Pressure p_water_in = 2.1e5 "Start water inlet pressure" annotation(Dialog(group = " Cooling"));
  parameter .Modelica.Units.SI.Pressure p_water_out = 1.7e5 "Water outlet pressure" annotation(Dialog(group = " Cooling"));
  ...
  parameter .Modelica.Units.SI.Current I_min = 180 "Minimum value of load current" annotation(Dialog(group = "Load"));
  parameter .Modelica.Units.SI.Current I_max = 450 "Maximum value of load current" annotation(Dialog(group = "Load", enable = not constant_current));
  ...
  .FuelCell.Sources.GasFlowBoundary flowAnode(
    ...
    T = T_an_in,
    use_flow_in = true,
    use_Th_in = false,use_X_in = false) annotation (Placement(transformation(
  ...
  .FuelCell.Sources.GasPressureBoundary sinkCathode(
    redeclare package Medium = Medium_cath,
    p = p_cath_out,
    T = T_cath_in,X = X_cath) annotation (Placement(transformation(extent={{99.61496612607814,-30.38503387392182},{79.61496612607814,-10.38503387392182}},
          rotation=0.0,origin = {0.0,0.0})));
  ...
  .PEMFCModel.Stacks.PEMFC.Empirical.CoolStack coolStack(
    ...
    X_feed_an = X_an,
    X_feed_cath = X_cath, X_start_anode = flowAnode.X,m_flow_start_anode = m_flow_an_min,p_start_out_anode = sinkAnode.p,p_start_in_anode = p_an_in,X_start_cathode = flowCathode.X,m_flow_start_cathode = m_flow_cath_min,p_start_out_cathode = sinkCathode.p,p_start_in_cathode = p_cath_in,T_start_in_anode = flowAnode.T,T_start_in_cathode = flowCathode.T,T_start_out_cathode = flowCathode.T,T_start_out_anode = flowAnode.T,positiveFlow_cooling = true,from_dp_cooling = false,positiveFlow_anode = true,positiveFlow_cathode = true,from_dp_anode = false,from_dp_cathode = false,p_start_in_cooling = p_water_in,T_start_in_cooling = sourceW.T,T_start_out_cooling = sourceW.T,m_flow_start_cooling = m_flow_water_min, h_inflow_an = flowAnode.fluidPort.h_outflow, h_inflow_cath = flowCathode.fluidPort.h_outflow, h_inflow_cooling = sourceW.fluidPort.h_outflow,p_start_out_cooling = sinkP.p,enable_setting = true,dp_smooth = 1e-2,mflow_smooth = 1e-8,dp_smooth_cooling = 1e-2,mflow_smooth_cooling = 1e-8,redeclare replaceable model Friction_anode = .Modelon.ThermoFluid.FlowChannels.PipeResistances.SinglePhase.LinearOperatingPointLoss(d0 = 0.4,dp0 = 0.1e5,m_flow0 = m_flow_an_min),redeclare replaceable model Friction_cathode = .Modelon.ThermoFluid.FlowChannels.PipeResistances.SinglePhase.LinearOperatingPointLoss(d0 = 1.2,dp0 = 0.15e5,m_flow0 = m_flow_cath_min),redeclare replaceable model HeatTransfer_anode = .Modelon.ThermoFluid.FlowChannels.HeatTransfer.SinglePhase.SinglePhase,redeclare replaceable model HeatTransfer_cathode = .Modelon.ThermoFluid.FlowChannels.HeatTransfer.SinglePhase.SinglePhase,redeclare replaceable model Friction_cooling = .Modelon.ThermoFluid.FlowChannels.PipeResistances.TwoPhase.DensityProfileFriction,redeclare replaceable model HeatTransfer_cooling = .Modelon.ThermoFluid.FlowChannels.HeatTransfer.TwoPhase.SimpleTwoPhase,N = 5,A_cell = 300e-4,L_anode = 0.0813,D_anode = 0.023,D_cathode = 0.0445,L_cathode = 0.0813,L_cooling = 0.0813,D_cooling = 0.0375,diffusiveSpecies = {"O2","N2"},n_cell = 11,M_stack = 10.1,z = 75e-6,j_0 = 10,m_conc = 3.4e-4,alpha = 0.5,c1 = 5.75e-3,c_stack = 10e8) annotation (Placement(transformation(extent={{-22.0,-2.0},{18.0,38.0}},rotation = 0.0,origin = {0.0,0.0})));

   .FuelCell.Sources.WaterFlowBoundary sourceW(redeclare package Medium = MediumWater, T = T_water_in,use_flow_in = true,use_Th_in = false) annotation (Placement(transformation(extent={{-57.8821596637993,-50.0},{-37.8821596637993,-30.0}},rotation = 0.0,origin = {0.0,0.0})));
  ...
  .Modelica.Blocks.Sources.Ramp ramp2(
    ...
    duration = if constant_current then 0 else simulation_time,
    startTime = if constant_current then simulation_time else 0) annotation(Placement(transformation(extent = {{-125.08019412312179,40.33433323817416},{-113.24549556331267,52.16903179798327}},origin = {0.0,0.0},rotation = 0.0)));
...
equation       
  ...
  connect(multiDisplaySensor4.portA, sourceW.fluidPort) annotation (Line(points={{-32,-40},{-38.8821596637993,-40}}, color={0,0,255}));
  ...
  connect(ramp.y,sourceW.m_flow_in) annotation(Line(points = {{-70.28038185111427,-42.54436907142595},{-61.95890105565714,-42.54436907142595},{-61.95890105565714,-30},{-53.8821596637993,-30}},color = {0,0,127}));
  connect(ramp2.y,flowAnode.m_flow_in) annotation(Line(points = {{-112.65376063532221,46.25168251807872},{-97.93906632502666,46.25168251807872},{-97.93906632502666,39.768425703538625}},color = {0,0,127}));
