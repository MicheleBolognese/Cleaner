...
model CoolStack "PEMFC stack model with cooling"
  ...
  extends .PEMFCModel.Stacks.Templates.CoolStack(
    redeclare model SubStack = .PEMFCModel.Stacks.PEMFC.Empirical.SubStack,
    subStack(
    c1 = c1,
    alpha = alpha,
    j_0 = j_0,
  ...
  parameter Real c1(unit = "V/K") = 0.85e-3 "Voltage derivative by temperature" annotation (Dialog(enable = enable_setting,tab = "Substack",group = "Characteristics and polarization"));
  ...
  parameter .Modelica.Units.SI.CurrentDensity j_0 = 1 "Exchange current density for activation loss" annotation (Dialog(enable = enable_setting,tab = "Substack",group = "Characteristics and polarization"));
  parameter .Modelica.Units.SI.CurrentDensity j_loss = 100 "Activation current density loss" annotation (Dialog(enable = enable_setting,tab = "Substack",group = "Characteristics and polarization"));
  parameter .Modelica.Units.SI.Voltage m_conc = 3e-4 "Pre-exponential factor for concentration loss" annotation (Dialog(enable = enable_setting,tab = "Substack",group = "Characteristics and polarization"));
  parameter Real n_conc = 3.2e-4 " Exponential factor for concentration loss" annotation(Dialog(group = "Characteristics and polarization",tab = "Substack",enable = enable_setting));
