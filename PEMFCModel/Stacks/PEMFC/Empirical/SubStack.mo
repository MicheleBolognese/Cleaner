within PEMFCModel.Stacks.PEMFC.Empirical;
model SubStack "PEMFC substack"
    
  extends .PEMFCModel.Stacks.Templates.SubStack(
    redeclare model Membrane = .PEMFCModel.Membrane.PEMFC.Empirical, 
    cell(
  ...
  parameter Real c1(unit = "V/K") = 0.85e-3 "Voltage derivative by temperature" annotation(Dialog(enable = enable_setting,tab = "Cell model",group = "Characteristics and polarization"));
  ...
  parameter .Modelica.Units.SI.CurrentDensity j_0 = 1 "Exchange current density for activation loss" annotation (Dialog(enable = enable_setting,tab = "Cell model",group = "Characteristics and polarization"));
  parameter .Modelica.Units.SI.CurrentDensity j_loss = 100 "Activation current density loss" annotation (Dialog(enable = enable_setting,tab = "Cell model",group = "Characteristics and polarization"));
  parameter .Modelica.Units.SI.Voltage m_conc = 3e-4 "Pre-exponential factor for concentration loss" annotation (Dialog(enable = enable_setting,tab = "Cell model",group = "Characteristics and polarization"));
  parameter Real n_conc = 3.2e-4 " Exponential factor for concentration loss" annotation(Dialog(group = "Characteristics and polarization",tab = "Cell model",enable = enable_setting));
